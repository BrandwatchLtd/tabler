        ...
        attach: function(table){
            ...
            table.renderHead = function(data, spec){
                var html = ['<tr class=toggleColumns>', '<th colspan="' + spec.length + '">',
                        ...
                        '<a class="help icon-bwhelp tinymargin-left" data-key="table-show-hide-columns"></a>',
                        '</th>', '</tr>'];
                ...
                $toggleUI.find('ul.columns').html(_(table.toggleColumns.customColumns).chain()
                        ...
                        .map(function(group, name){
                            ...
                            if(name){
                                ...
                                html.push('<label for="' + _.escape(id) + '">' + _.escape(name) + '</label>');
                                html.push('<ul>');
                            }

                            html = html.concat(_(group).map(function(spec){
                                var id = _.uniqueId('toggleColumnsUIValue');
                                return ['<li><input name=column type=checkbox value="', _.escape(spec.id || spec.name), '" id="', _.escape(id), '" ',
                                                (spec.disabled ? '' : ' checked'),
                                            ' />',
                                            '<label for="',
                                                id,
                                            '">', _.escape(spec.name || spec.field || spec.id), '</label>',
                                        '</li>'].join('');
                            }));
