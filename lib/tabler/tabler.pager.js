(function(root, factory){
    ...
    _.extend(Pager.prototype, {
        ...
        **/
        ...
        renderStandalonePager: function(table, data){
            var pageSize = this.pageSize;
            var currentPage = this.currentPage;
            var totalResults = data.totalResults || data.items.length;
            var totalPages = this.totalPages = Math.ceil(totalResults / pageSize);
            var pagerSpec = [];
            var cssClass = this.cssClass ? 'pager pager-collapsed ' + this.cssClass : 'pager pager-collapsed';
            var pagerHtml = ['<ol class="'+ cssClass + '">'];

            this.$pagerEl = $(document.createElement('div')).addClass('standalone-pager');
            ...
            this.$pagerEl.append(_.flatten(pagerHtml.concat(['</ol>', []])).join('\n'));
            this.attachPaginationEvent(this.$pagerEl);
            return this.$pagerEl;
        },
        ...
        attach: function(table){
            ...
            var self = this,
                ...
                renderStandalonePaging = this.originalRenderStandalonePaging = table.renderStandalonePaging,
                renderFootHTML,
                standalonePagingHTML,
            ...
            if(table.renderFoot){
                table.renderFoot = function(data, spec){
                    renderFootHTML = renderFoot.call(this) || '';
                    return renderFootHTML + self.renderPager(this, data, spec);
                };
            }

            if(table.renderStandalonePaging){
                table.renderStandalonePaging = function(data, spec){
                    return self.renderStandalonePager(this, data);
                }
            }

            this.attachPaginationEvent(table.$el);
        },
        attachPaginationEvent: function($container){
            var self = this;
            $container.on('click', 'ol.pager li', function(e) {
                e.preventDefault();
        ...
        detach: function(table){
            ...
            if(this.$pagerEl) {
                this.$pagerEl.off('click', 'ol.pager li');
            }
        }
    });
