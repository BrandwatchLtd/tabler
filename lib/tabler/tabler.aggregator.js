(function(root, factory){
    ...
    _.extend(Aggregator.prototype, {
        attach: function(table){
            var formatValue = this.originalFormatValue = table.formatValue;
            ...
            table.renderFoot = function(data, spec){
                var foot = renderFoot.apply(this, arguments),
                    defaultAttrs = {
                        'class': 'tablecell tabletotalcell'
                    };
                if(_(spec).any(function(col){return !!col.aggregator;})){
                    foot = ['<tr>'].concat(_(spec).map(function(spec){
                            if(spec.aggregatorText){
                                return table.makeTag('td', spec.aggregatorText, _.extend({}, defaultAttrs, table.makeColumnAttrs(spec)));
                            }
                            if(spec.aggregator){
                                return table.makeTag('td', spec._memo, _.extend({}, defaultAttrs, table.makeColumnAttrs(spec)));
                            }
                            return '<td class="' + defaultAttrs['class'] + '"></td>';
                        })).concat('</tr>')
                        .join('\n');
                }
                return foot;
