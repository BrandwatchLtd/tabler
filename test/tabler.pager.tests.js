define([
    ...
    describe('tabler.pager', function(){
        ...
        describe('with option to hide controls for single page', function(){

            beforeEach(function(){
                table.pager.hideWhenOnePage = true;
            });

            describe('when there\'s one page', function(){
                it('adds no pager ol when there\'s just one page', function(){
                    table.pager.pageSize = rowData.length + 1;
                    table.render();
            ...
            describe('when there\'s many pages', function(){
                it('adds pager ol to the tfoot of the table', function(){
                    ...
                    table.pager.pageSize = Math.ceil(rowData.length / 2);
        ...
        describe('with many pages', function(){
            ...
            it('shows current page li with a class of current', function(){
                // Set the pageSize so that we get three pages
                table.pager.pageSize = Math.ceil(rowData.length / 3);
                table.render();
            ...
            it('can page through results by clicking on page links', function(){
                ...
                expect(table.$('tbody tr').eq(0).find('td').eq(0).text()).toEqual('a2');
                expect(table.$('tbody tr').eq(0).find('td').eq(1).text()).toEqual('b2');
                expect(table.$('tbody tr').eq(1).find('td').eq(0).text()).toEqual('a3');
                expect(table.$('tbody tr').eq(1).find('td').eq(1).text()).toEqual('b3');

                table.$('tfoot tr td ol.pager li[data-page=2]').click();

                expect(table.$('tbody tr').eq(0).find('td').eq(0).text()).toEqual('a4');
                expect(table.$('tbody tr').eq(0).find('td').eq(1).text()).toEqual('b4');
            });
